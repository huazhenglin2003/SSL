...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file):
        ...
        Also returns hallucinated tokens and their indices for further analysis.
        output, hallucinated_tokens_info = compute_chair(cap_file)
        """
        ...
        for i, cap_eval in enumerate(caps):
            ...
            cap_dict["metrics"]["CHAIRi"] = (
                len(cap_dict["mscoco_hallucinated_words"]) / float(len(words))
                if len(words) > 0
                else 0.0
            )
